...
const BOUNDSACTION  BA_STOP   = 0,
                    ...
                    BA_BOUNCE = 2,
                    BA_DIE    = 3;
typedef WORD SPRITEATTRIBUTE;
const SPRITEATTRIBUTE SAT_PLAYER = 99,
					  SAT_ENEMY =98,
					  SAT_BOSS = 97,
					  SAT_ENEMYBULLET = 96,
					  SAT_PLAYERBULLET = 95,
...
protected:
  ...
  BOOL          m_bDying;
  BOOL          m_bOneCycle;
  SPRITEATTRIBUTE m_satAttribute;
  time_t		m_tStarttime;
  time_t		m_tLifetime;
  int			m_iHp;
  double		m_dDamage;
...
public:
  ...
  double SufferDamage(double power) { m_dDamage=m_dDamage+power;return m_dDamage;};
  double GetDamage() { return m_dDamage;}
  bool DamageEnough() {return m_dDamage>m_iHp;};
  Bitmap* GetBitmap()               { return m_pBitmap; };
  void    SetNumFrames(int iNumFrames, BOOL bOneCycle = FALSE);
  void	  AddFrames();
  void	  SetFrames(int i)  { m_iCurFrame = i;};
  ...
  void    SetSpriteAttribute(SPRITEATTRIBUTE sat) { m_satAttribute = sat; }
  SPRITEATTRIBUTE    GetSpriteAttribute() { return m_satAttribute;}
...
//-----------------------------------------------------------------
void Sprite::SetCollisionRect(RECT rect)
{
	CopyRect(&m_rcCollision,&rect);
}
